...
abstract class AbstractSslCommerz implements SslCommerzInterface
{
    ...
    public function formatResponse($response, $type = 'checkout', $pattern = 'json')
    {
        ...
        if (!empty($sslcz['GatewayPageURL'])) {
            if (!empty($this->getApiUrl()) && strpos($this->getApiUrl(), 'securepay.sslcommerz.com') !== false) {
            ...
            if (strpos($sslcz['failedreason'] ?? '', 'Store Credential') === false) {
                $message = $sslcz['failedreason'] ?? 'Unknown error';
            } else {
                $message = "Check SSLCZ_TESTMODE and SSLCZ_STORE_PASSWORD in .env — do NOT use Merchant Panel password.";
            }
